...
try:
    csv_file_pos = open("upstreamTRNApos.bed", 'r')
    csv_file_neg = open("upstreamTRNAneg.bed", 'r')
    ...
    with open('S288c_fixed.gff') as tRNAs:
        ...
        csv_file_pos = open("upstreamTRNApos.bed", 'w')
        csv_file_neg = open("upstreamTRNAneg.bed", 'w')
        for l, line in enumerate(tRNA_reader):
            if line[2] == "tRNA_gene":
                if line[6] == "+":
                    csv_file_pos.write(line[0])
                    csv_file_pos.write('\t')
                    csv_file_pos.write(str(int(line[3])-1000))
                    csv_file_pos.write('\t')
                    csv_file_pos.write(str(line[3]))
                    csv_file_pos.write('\t')
                    ...
                    csv_file_neg.write('\t')
                    csv_file_neg.write(str(line[4]))
                    csv_file_neg.write('\t')
                    csv_file_neg.write(str(int(line[4]) + 1000))
                    csv_file_neg.write('\t')
                    gene_info = line[8].split(';')
                    ID_raw = gene_info[0]
                    id_tag = ID_raw.split('=')[1]
